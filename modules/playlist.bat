...
set "output_template=%DOWNLOAD_DIR%\playlists\%%(playlist_index)s - %%(title)s.%%(ext)s"
...
yt-dlp.exe %ytdlp_base_args% %playlist_args% -f "%format_selection%" -o "%output_template%" --write-thumbnail --embed-thumbnail --embed-metadata %playlist_range% %aria2c_args% %hw_accel_opts% "%link%" || (
